# -*- coding: utf-8 -*-
"""
Created on Mon Mar 27 17:23:38 2023
...
title = "(w_inv,ln,vgg13_bn,adam,dice,d=500,lr=1e-6,e=100)"
...
if resume_train:
    title = title + ",e=50)"
    train_loss_history = os.path.join(
        saved_history, "t_losses_" + title + ".pkl")
    valid_loss_history = os.path.join(
        saved_history, "v_losses_" + title + ".pkl")
    iou_score_history = os.path.join(
        saved_history, "iou_score_" + title + ".pkl")
    pixel_accuracy_history = os.path.join(
        saved_history, "accuracy_" + title + ".pkl")

# Image paths
train_dir = r"C:\Users\GPU\particle_size_project\Dataset\wood_mix_500\train\Images\*.png"
train_maskdir = r"C:\Users\GPU\particle_size_project\Dataset\wood_mix_500\train\Masks\*.png"
val_dir = r"C:\Users\GPU\particle_size_project\Dataset\wood_mix_500\valid\Images\*.png"
val_maskdir = r"C:\Users\GPU\particle_size_project\Dataset\wood_mix_500\valid\Masks\*.png"
# --------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    # model = smp.UnetPlusPlus(encoder_name='timm-regnetx_120', encoder_depth=5,
    #               encoder_weights='imagenet', decoder_use_batchnorm=True,
    #               decoder_channels=(512, 256, 128, 64, 32), decoder_attention_type=None,
    #               in_channels=1, classes=1, activation=None, aux_params=None).to(device)

    model = smp.MAnet(encoder_name='vgg13_bn', encoder_depth=5, encoder_weights='imagenet',
                      decoder_use_batchnorm=True, decoder_channels=(512, 256, 128, 64, 32),
                      decoder_pab_channels=64, in_channels=1,
                      classes=1, activation=None, aux_params=None).to(device)

    # model = smp.Linknet(encoder_name='timm-regnetx_160', encoder_depth=5,
    #                     encoder_weights='imagenet', decoder_use_batchnorm=True,
    ...
    if train:
        ...
        train_transform = ComposeDouble([
            ...
            FunctionWrapperDouble(create_dense_target,
                                  input=False, target=True),
        ...
        train_dataset = SegmentationDataSet(
            imgs_dir=train_dir, masks_dir=train_maskdir, transform=train_transform)
        val_dataset = SegmentationDataSet(
            imgs_dir=val_dir, masks_dir=val_maskdir, transform=validate_transform)
    ...
    if plot:
        plot_training(title, training_losses, validation_losses,
                      iou_score, pixel_accuracy, f1_score, precision, recall, scale)
