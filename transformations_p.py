# -*- coding: utf-8 -*-
"""
Created on Sun Oct 23 17:54:36 2022
...
def img_preprocess_grayscale(images_path_string):
    ...
    for idx, fname in enumerate(images_path):
        ...
        if re.search(r'mask', fname):
            ...
            # idx=idx+1
            # saving the image        
            cv2.imwrite(r"small_sample/plastic/(t=0){}.png".format(idx+1), img)
            pass
        else:
            # img = cv2.medianBlur(img, 1)
            # clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
            # img = clahe.apply(img)
            # img = cv2.resize(img, (736,1280),interpolation=cv2.INTER_AREA)
            # saving the image        
            # print(img.shape)
            # cv2.imwrite(r"results/labeled/plastic/labeled1_gray.png", img)
            img = cv2.medianBlur(img, 1)
            # img = cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE)
            # img = cv2.resize(img,(1280,736))
            # cv2.imwrite(r"results/unlabeled/plastic/unlabeled2_blur.png", img)
            clahe = cv2.createCLAHE(clipLimit=50.0)
            img = clahe.apply(img)
            # cv2.imwrite(r"results/unlabeled/plastic/unlabeled2_clahe.png", img)
            # cv2.imwrite(r"labaled..{}.png".format(idx+1), img)
